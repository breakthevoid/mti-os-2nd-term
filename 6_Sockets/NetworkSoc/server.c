...
{
    setbuf(stdout, NULL); // Set stdout to unbuffered (for logging)
    ...
    // Создание сокета (флаг AF_INET - сетевой сокет)
    ...
    serv_addr.sin_addr.s_addr = INADDR_ANY;     // Использовать любой адрес
